...
const Header: React.FC = () => {
  ...
  // Detector invisível no topo (só na página inicial)
  React.useEffect(() => {
    if (!isHomePage) return;
    
    const handleMouseMove = (e: MouseEvent) => {
      if (e.clientY <= 60 || isHeaderHovered) {
        setShowHeader(true);
      } else {
        setShowHeader(false);
      }
  ...
  return (
    <>
      {/* Header visível só quando mouse está no topo */}
      <header
        className={`transition-all duration-300 bg-black ${
          showHeader ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-10 pointer-events-none'
        }`}
        style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 49 }}
          ...
          <div className="container mx-auto px-6 flex justify-between items-center text-sm">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span>Rua das Artes Marciais, 123 - Centro</span>
              </div>
            </div>
            <div className="hidden md:flex items-center space-x-4">
              <span>Horário: Seg-Sex 06:00-22:00</span>
            </div>
          </div>
        </div>
