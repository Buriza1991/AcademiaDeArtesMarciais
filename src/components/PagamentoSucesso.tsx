            ...
            {state?.userData && (
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-gray-300">
                    <p><span className="text-white">Método:</span> {state?.paymentMethod ? paymentMethodNames[state.paymentMethod as keyof typeof paymentMethodNames] : 'Não informado'}</p>
                    <p><span className="text-white">Status:</span> {state?.paymentMethod === 'boleto' ? 'Aguardando pagamento' : 'Aprovado'}</p>
                    <p><span className="text-white">Data:</span> {new Date().toLocaleDateString('pt-BR')}</p>
                  </div>
                </div>
              </div>
            )}
